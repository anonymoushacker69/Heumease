import speech_recognition as sr # to recognise
...
def respond(voice_data):
    ...
    if there_exists(["what is your name","what's your name","tell me your name"]):
        if person_obj.name:
            ...
            engine_speak("My name is Heumease . what's your name?")
    ...
    if there_exists(["amazon.in"]):
        search_term = voice_data.split("for")[-1]
        url="https://www.amazon.in"
        webbrowser.get().open(url)
        engine_speak("here is what i found for on amazon.in")
    ...
    if there_exists(["open excel"]):
        ...
        webbrowser.get().open(url)
        engine_speak("opening excel")

        #open powerpoint
    if there_exists(["open powerpoint"]):
        search_term=voice_data.split("for")[-1]
        url="https://office.live.com/start/PowerPoint.aspx"
        webbrowser.get().open(url)
        engine_speak("opening powerpoint")
    ...
    #open gmail
    if there_exists(["open gmail","gmail","check my gmail"]):
        search_term = voice_data.split("for")[-1]
        ...
        #open yahoo
    if there_exists(["open yahoo","yahoo","check my yahoo mail"]):
        search_term = voice_data.split("for")[-1]
        url="https://login.yahoo.com/"
        webbrowser.get().open(url)
        engine_speak("here you can check your yahoo mail")

        #open teams
    if there_exists(["open teams","teams","microsoft teams"]):
        search_term = voice_data.split("for")[-1]
        url="https://www.microsoft.com/en-ww/microsoft-teams/log-in"
        webbrowser.get().open(url)
        engine_speak("here you can check your teams account")

        #open outlook
    if there_exists(["open outlook mail","outlook","check my outlook"]):
        search_term = voice_data.split("for")[-1]
        url="https://outlook.live.com/"
        webbrowser.get().open(url)
        engine_speak("here you can check your outlook account")

        #open rediff
    if there_exists(["open rediff mail","rediff","check my rediff mail"]):
        search_term = voice_data.split("for")[-1]
        url="https://mail.rediff.com/cgi-bin/login.cgi"
        webbrowser.get().open(url)
        engine_speak("here you can check your rediff mail")

        #open office 365
    if there_exists(["open office 365","office 365","check office 365"]):
        search_term = voice_data.split("for")[-1]
        url="https://login.microsoftonline.com/"
        webbrowser.get().open(url)
        engine_speak("here you can check your office 365")
    ...
    if there_exists(["capture","my screen","screenshot","take a screenshot"]):
        ...
        myScreenshot.save('C:/Users/dell/Pictures/Screenshots/file name.png')
        engine_speak("Screenshot Captured in screenshot folder,Pictures")
    ...
    if there_exists(["exit", "quit", "goodbye", "stop", "bye", "tata", "close"]):
        engine_speak("we could continue more , but.,,...,,,,,..,,,,, byee.             Developed by Aadesh ")
        exit()
...
while(1):
    ...
    voice_data = record_audiovoice_data = record_audio("I am Heumease, What can i do for you ") 
    print("Done")
    print("Q:", voice_data)
